...
std::filesystem::path InfoPopup::getPathInPack(const char* filename) const {
    std::string suffix;
...
bool InfoPopup::setup(std::shared_ptr<Pack> pack) {
    m_pack = pack;
...
InfoPopup* InfoPopup::create(const std::shared_ptr<Pack>& pack) {
